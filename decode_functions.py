...
def extract_format_information(matrix):
    ...
    for x in range(8):
        if x != 6:  # Skip the timing pattern
            format_info.append(matrix[8][x])

    for y in range(7, -1, -1):
        if y != 6:  # Skip the timing pattern
...
def extract_data(data_bits, mode, count):
    ...
    if mode == "Numeric":
        # Numeric mode: 3 digits per 10 bits
        data = ""
        for i in range(0, count * 10, 10):
            ...
            for bit in chunk:
                value = (value << 1) | bit
            data += f"{value:03}"  # Pad with leading zeros
        return data
    elif mode == "Byte":
        ...
        for i in range(0, count * 8, 8):
            ...
            for bit in chunk:
                value = (value << 1) | bit
            data += chr(value)  # Convert to ASCII
        return data
    # Add support for Alphanumeric and Kanji modes if needed
    elif mode == "Alphanumeric":
        # Alphanumeric mode: 2 characters per 11 bits
        data = ""
        alphanumeric_table = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:"
        i = 0
        while i < count * 11:
            # Read 11 bits for a pair of characters
            ...
            if i < count * 11:  # Add the second character if it exists
